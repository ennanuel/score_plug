    ...
    <div className="flex flex-col gap-4">
      {
        squad.map(({ title, players }) => (
          <div key={title} className="rounded-xl p-4 flex flex-col gap-4 bg-[#191919] border border-transparent">
            <h3 className='capitalize text-sm font-semibold text-white-100'>{/goalkeeper/.test(title) ? `${title}s` : title}</h3>
            <ul className="grid grid-cols-4 gap-2">
              {
                players.map((player) => (
                  <li key={player._id ? player._id : player.id ? player.id : player.name} className="flex items-center p-6 gap-4 bg-white-100/5 rounded-md">
