            ...
            <div className="flex flex-col gap-4">
                {
                    data.team.matches.length > 0 || data.teamNextFixtures.length > 0 ?
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="flex flex-col gap-6 rounded-xl bg-white-100/10 border border-transparent p-3 sm:p-6">
                                ...
                                <ul className="flex justify-between gap-4">
                                    {
                                        data.team.matches.map(({ _id, homeTeam, awayTeam, score }) => (
                                            <li key={_id} className="flex flex-col items-center justify-center gap-4">
                                                <span className={`${(String(homeTeam._id) === String(id) && score.fullTime.home > score.fullTime.away) || (String(awayTeam._id) === String(id) && score.fullTime.away > score.fullTime.home) ? 'bg-green-600' : score.fullTime.home === score.fullTime.away ? 'bg-gray-500' : 'bg-red-500'} px-2 h-6 relative flex items-center justify-center text-2xs font-semibold text-white-100 rounded-md after:absolute ${(String(homeTeam._id) === String(id) && score.fullTime.home > score.fullTime.away) || (String(awayTeam._id) === String(id) && score.fullTime.away > score.fullTime.home) ? 'after:bg-green-600' : ''} whitespace-nowrap after:-bottom-1 after:left-1/2 after:-translate-x-1/2 after:w-[calc(100%_-_16px)] after:h-[2px] after:rounded-full`}>{score.fullTime.home} - {score.fullTime.away}</span>
                            ...
                            <Link href={`/match/${(nextFixture ? nextFixture : data.team.matches[0])?._id}`} className="group flex flex-col gap-8 rounded-xl bg-white-100/10  border border-transparent px-3 pb-4 pt-3 sm:pt-4 sm:px-6 sm:pb-6">
