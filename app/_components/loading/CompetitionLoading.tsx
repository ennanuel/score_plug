        ...
        <ul className="flex flex-col border border-secondary-900/50 rounded-md overflow-hidden">
            {
                competitions.map((key) => (
                    ...
                    >
                        <span className={`${small ? 'w-6 h-6' : 'w-8 h-8'} animate-loadgradient loading  ml-2 rounded-full bg-secondary-900/50`}></span>
                        <span className='animate-loadgradient loading w-full max-w-[160px] h-4 rounded-sm bg-secondary-900/50'></span>
                        {!small ? <span className="animate-loadgradient loadingw-4 h-4 rounded-full bg-secondary-900/50"></span> : null}
                    </li>
                ))
            }
        </ul>
