...
const CompetitionCard = ({ _id, name, emblem, area, recentMatches }: Competition) => {
    return (
        <Link href={`/competition/${_id}`} className={`flex items-center gap-4 justify-between bg-primary-800 hover:bg-primary-400/50 hover:border-transparent p-2 rounded-sm border ${recentMatches.hasLiveMatch ? 'border-highlight-400/20' : 'border-transparent'}`}>
