...
const NavLink = ({ href, title, Icon, ActiveIcon, alert }: { href: string; title: string; Icon: IconType; ActiveIcon: IconType; alert: boolean; }) => {
    ...
    return (
        <Link href={href} className={`relative h-[30px] flex items-center justify-center gap-2 px-3 rounded-md text-sm ${active ? 'bg-secondary-100 text-black-900 font-semibold' : 'bg-primary-100/10 text-secondary-600 hover:bg-primary-200/20 hover:text-white-200'}`}>
