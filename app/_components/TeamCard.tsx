...
function TeamCard ({ _id, name, crest, hasOngoingMatch }: Team) {
    return (
        <Link href={`/team/${_id}`} className={`relative flex flex-col items-center justify-center p-3 gap-3 bg-primary-600 rounded-md hover:bg-primary-500/80 border border-transparent ${hasOngoingMatch && 'border-highlight-400/50 hover:border-transparent'}`}>
            {
                hasOngoingMatch ?
