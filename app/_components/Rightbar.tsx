...
const Rightbar = () => {
  ...
  if(error) return (
    <div className="flex flex-col items-center justify-center gap-1 py-20 px-6 rounded-xl border border-transparent bg-white-100/10 h-fit">
      <span className="text-white-600 text-sm font-semibold">Oh no!</span>
      <p className="text-xs text-white-800 text-center">Request failed to load</p>
    </div>
  )
            ...
            <Link href={`match/${featuredPrediction._id}`} className="flex flex-col gap-2 p-2 pt-0">
              <div className="grid grid-cols-[1fr,_auto,_1fr] gap-2 shadow-lg p-2 min-h-20 rounded-lg bg-white-100/5">
                <div className="flex flex-col justify-center items-center gap-1">
                    <Image src={featuredPrediction.homeTeam.crest || String(process.env.NEXT_IMAGE_URL)} width={40} height={40} alt={featuredPrediction.homeTeam.name} className="aspect-square object-contain" />
                ...
                <div className="flex flex-col justify-center items-center gap-1">
                    <Image src={featuredPrediction.awayTeam.crest || String(process.env.NEXT_IMAGE_URL)} width={40} height={40} alt={featuredPrediction.awayTeam.name} className="aspect-square object-contain" />
