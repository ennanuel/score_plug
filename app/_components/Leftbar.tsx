...
const Leftbar = () => {
  ...
  return (
    <div className="col-span-1 sticky top-[50px] flex flex-col gap-4">
      <div className="pb-2 pt-4 rounded-lg border border-white-100/10 bg-white-100/5 flex flex-col gap-4">
        <h2 className="font-semibold text-white-300 text-sm px-4">Top Leagues</h2>
        {
          loading ?
                  ...
                  ?.map((competition) => (
                    <li key={competition._id} className="">
                      <Link href={`/competition/${competition._id}`} className="flex items-center h-9 px-4 gap-4 hover:bg-white-100/10">
                        ...
                        <span className={`text-xs ${competition.recentMatches.hasLiveMatch ? 'text-yellow-500' : 'text-white-600'}`}>{competition.name}</span>
      ...
      <div className="rounded-lg border border-white-100/10 bg-white-100/5 flex flex-col">
        <button className="text-white-500 hover:text-white-600 flex items-center gap-3 p-4">
          <span className="font-semibold text-sm ">All Leagues</span>
          <BiCaretDown size={14} />
        </button>
      </div>
    </div>
